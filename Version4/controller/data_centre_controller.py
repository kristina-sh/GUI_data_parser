...
class DataCentreController():
    ...
    def __init__(self, root):
        ...
        options = ["Power Consumption Rate (kVA) Horizontal", "Power Consumption Rate (kVA) Vertical",
                   "Contracted Power Capacity (kVA) Horizontal", "Contracted Power Capacity (kVA) Vertical"]
        # Variable to store selected option
        selected_option = tk.StringVar(self.root)
        selected_option.set(options[0])  # Set the default option
        dropdown = tk.OptionMenu(root, selected_option, *options, command=lambda value: self.on_select(value))
        dropdown.pack(pady=5)

    def on_select(self, value):
        self.view.display_text(value)
        chart_title = value.rsplit(' ', 1)[0]
        chart_data = self.datastore.create_data_for_chart(chart_title)
        if "Horizontal" in value:
            self.view.draw_horizontal_chart(chart_title, chart_data)
        else:
            self.view.draw_vertical_chart(chart_title, chart_data)
